...
// through whatever it has stored without checking first.
//
...
// second one for the same new name. The change is logged at warn level, and
// the resolved spec — record id included — is what LIBTUNNEL_SPEC carries.
//
...
func From(spec string) TunnelV1 {
	return v1alpha1.From(spec, func(backend string, raw json.RawMessage, metadata v1.SpecMetadata) (v1.Tunnel, error) {
		switch backend {
		case "":
			return New(Cloudflare()), nil
		case "cloudflare":
			s := &cloudflare.Spec{}
			if err := v1alpha1.Unpack(raw, metadata, s); err != nil {
